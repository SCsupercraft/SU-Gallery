...
export function SearchableExtensionGrid({
  ...
  const [params, setParams] = useState<SearchParams>(
    useUrlSearchParams
      ? getDefaultParamsFromURL(new URLSearchParams(window.location.search))
      : getDefaultParams(),
  );
        ...
        <div className="flex-col flex-auto xl:mt-0 mt-1.5 xl:ml-auto xl:mr-0 mr-auto w-fit xl:pt-3 pb-3 xl:justify-items-end justify-items-start">
          ...
          {extensionManager.getGalleries().length > 1 && (
            <Gallery
              params={params}
              setParams={setParams}
              galleries={extensionManager.getGalleries().map((gallery) => {
                return gallery.name;
              })}
            />
          )}
    ...
    <div className="inline-block ml-1 h-full align-middle">
      <ToggleGroup
        ...
        onValueChange={(value) => {
          setParams({
            ...params,
            includeDescription: value.includes('includeDescription'),
...
function Author({
  ...
  const [authorName, setAuthorName] = React.useState(
    authors
      .filter((author) => author.toLowerCase() === params.author.toLowerCase())
      .pop() || '',
  );
                ...
                {authors.map((author) => (
                  <CommandItem
                    ...
                    onSelect={(currentValue: string) => {
                      setAuthorName(
                        currentValue.toLowerCase() === authorName.toLowerCase()
                          ? ''
                          : currentValue,
                      );
                      setOpen(false);
...
function Gallery({
  ...
  const [galleryName, setGalleryName] = React.useState(
    galleries
      .filter(
        (gallery) => gallery.toLowerCase() === params.gallery.toLowerCase(),
      )
      .pop() || '',
  );
              ...
              <CommandGroup>
                {galleries.map((gallery) => (
                  <CommandItem
                    ...
                    onSelect={(currentValue: string) => {
                      setGalleryName(
                        currentValue === galleryName ? '' : currentValue,
                      );
                      setOpen(false);
                    }}
                  >
                    ...
                    <Check
                      className={cn(
                        'ml-auto',
                        galleryName === gallery ? 'opacity-100' : 'opacity-0',
                      )}
                    />
                  </CommandItem>
                ))}
    ...
    <div className="inline-block mr-3 h-full align-middle">
      ...
      >
        <PopoverTrigger asChild>
          ...
          >
            {supportedMod
              ? extensionManager.getSupportedMod(supportedMod)!.name
              : 'All mods'}
            <ChevronsUpDown className="opacity-50" />
          </Button>
        </PopoverTrigger>
            ...
            <CommandList>
              ...
              <CommandGroup>
                {supports.map((mod) => (
                  ...
                  >
                    {extensionManager.getSupportedMod(mod)!.name}
                    <Check
                      className={cn(
                        'ml-auto',
                        supportedMod.toLowerCase() === mod.toLowerCase()
                          ? 'opacity-100'
                          : 'opacity-0',
                      )}
                    />
...
function matchesSearch(extension: Extension, searchParams: SearchParams) {
  ...
  if (searchParams.gallery != '' && searchParams.gallery != extension.gallery) {
    return false;
  }
  if (
    searchParams.supports != '' &&
