...
> {
  ...
  protected processCommonModifications(
    extensions: GalleryJSON_Extension[],
  ): GalleryJSON_Extension[] {
    return this.processMarkAsDupe(this.processAddSupportedMod(extensions));
  }

  protected processMarkAsDupe(
    extensions: GalleryJSON_Extension[],
  ): GalleryJSON_Extension[] {
    const dupes =
      this.getModifications<GalleryModification_MarkExtensionAsDupe>(
        'duplicate',
      )
        .map((m) => m.extensionId)
        .flat();
    ...
    for (const extension of extensions) {
      if (dupes.includes(extension.id)) extension.duplicate = true;
    }
  ...
  protected processAddSupportedMod(
    ...
    for (const modification of this.getModifications<GalleryModification_AddSupportedMod>(
      ...
      for (const extension of extensions) {
        ...
        if (modification.uncertain) extension.maySupport.push(modification.id);
...
export type GalleryModification_MarkExtensionAsDupe = {
  type: 'duplicate';
