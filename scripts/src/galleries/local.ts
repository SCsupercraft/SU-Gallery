...
> {
  ...
  public constructor(data: GalleryData, modifications: GalleryModification[]) {
...
class LocalExtension {
    ...
    if (this.versions) {
      for (const version of this.versions) {
        const codeFile = path.resolve(
          extensionDirectory,
          version.foldername,
          this.extensionId.substring(this.extensionId.lastIndexOf('/') + 1) +
            '.js',
        );
  ...
  private readMetadata(data: LocalExtensionMetadata) {
    ...
    if (data.versions) {
      ...
      if (data.mainVersion) {
        if (typeof data.mainVersion != 'string')
          throw `Failed to load extension data for extension '${this.extensionId}'. Expected mainVersion to be a string.\nFound: ${data.mainVersion}`;
        this.mainVersion = data.mainVersion;
      }
    }
