...
export class LocalExtensionData extends ExtensionData {
	/**
		...
		return new Promise(async (resolve, reject) => {
			try {
				await BuildHelper.copy(
					path.resolve(
						process.cwd(),
						`${this.gallery.location}/code/${this.id}.js`
					),
					path.resolve(
		...
		return new Promise(async (resolve, reject) => {
			try {
				...
				await BuildHelper.copy(
					path.resolve(
						process.cwd(),
						`${this.gallery.location}/banner/${this.id}.${this.banner}`
					),
					path.resolve(
						BuildHelper.prebuildDirectory,
	...
	async findBannerExtension() {
		...
		for (let i in extensions) {
			...
			const exists = await BuildHelper.exists(
				path.resolve(
					process.cwd(),
					`${this.gallery.location}/banner/${this.id}.${extension}`
				)
			);
			if (exists) {
...
export class LocalExtensionGalleryData extends ExtensionGalleryData {
	/**
	...
	async addIcon() {
		return new Promise(async (resolve, reject) => {
			try {
				...
				await BuildHelper.copy(
					path.resolve(process.cwd(), this.iconLocation),
					path.resolve(
						BuildHelper.prebuildDirectory,
