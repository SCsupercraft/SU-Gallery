...
/******/ (() => {
	...
	(function (Scratch) {
		...
		if (!Scratch.extensions.unsandboxed) {
			throw new Error(
				'Debugging must be run unsandboxed to work properly!'
			);
		}
		...
		const buttonSRC =
			'https://raw.githubusercontent.com/SCsupercraft/projectloader/main/common%20assets/debugger%20icon.svg';
		...
		class Default {
			constructor() {
				this.CommandDescriptions = {
					...
					downloadLogs:
						'Downloads the logs to your computer as a file named "project.log".\n\tFirst argument is whether or not to include debug level logs',
				};
				this.Colors = {
					debug: '#1e90ff',
					log: '#ffffff',
		...
		const runHats = (opcode) => {
			...
			for (const target of vm.runtime.targets) {
				Object.values(target.blocks._blocks)
					...
					.forEach((block) => {
						threads.push(
							vm.runtime._pushThread(block.id, target, {
								stackClick: false,
							})
						);
					});
		...
		class debugging {
			...
			constructor(vm, runtime) {
				...
				this.runtime.on('EXTENSION_REMOVED', () => {
					if (
						vm.extensionManager.isExtensionLoaded('debugging') ===
						false
					) {
			...
			initializeExtension() {
				...
				this.newBtnUrl({
					NAME: 'Open debugger',
					HOVER: 'Opens the debugger',
					URL: buttonSRC,
					FUNC: this.openDebugger.bind(this),
				});
				...
				console.error = function (...args) {
					const errorToLog =
						typeof args[0] == 'object'
							? Cast.toString(args[0]['message'])
							: Cast.toString(args) || 'Unknown Error';
					if (
						!errorToLog.includes('at block debugging_error') &&
						!errorToLog.includes('at block debugging_fatalError')
					) {
						this._addLog(
							'Uncaught Error: ' + errorToLog,
				...
				return {
					...
					blocks: [
						...
						},
						{
							...
							text: [
								'run test [TEST] called [TEST_NAME] and if test succeeded',
								'else',
							],
							blockType: Scratch.BlockType.CONDITIONAL,
							branchCount: 2,
					...
					menus: {
						...
						PROTECTION_TYPES: {
							...
							items: ['Variable', 'List', 'Sprite', 'Broadcast'],
						},
						LOG: {
							acceptReporters: false,
							items: [
			...
			_getTargetFromMenu(targetName) {
				...
				let target =
					Scratch.vm.runtime.getSpriteTargetByName(targetName);
			...
			_addLog(log, style, debug) {
				...
				const logElement = this.consoleLogs.appendChild(
			...
			_addImgLog(imageSRC, style) {
				...
				const imageElement = this.consoleLogs.appendChild(
			...
			_parseCommand(command) {
				...
				this._addLog(
					'Split raw command: ' +
						JSON.stringify(rawCommand.split('')),
					`color: ${this.colors.debug};`,
					true
				); // color: dodgerblue;
				...
				this.debugFile +=
					'DEBUG Args: ' + JSON.stringify(data.args) + '\n';
				return data;
			}
			async _runCommand(parsedCommand) {
				...
				if (!this.isCommandEnabled({ COMMAND: command })) {
					this._addLog(
						'This command has been disabled by the project!',
						`color: ${this.colors.warning};`
					); // color: yellow;
					this.logFile +=
						'WARN This command has been disabled by the project!' +
						'\n';
					this.debugFile +=
						'WARN This command has been disabled by the project!' +
						'\n';
				...
				let customCommandExists = await this.runWhenCommandEntered({
					COMMAND: command,
					ARGS: args,
				});
				...
				switch (command) {
					case 'help': {
						if (args.length > 0) {
							...
							let explanation =
								'No description defined for this command.';
					...
					case 'start':
						try {
							...
							this._addLog(
								...
								`color: ${this.colors.log};`
							); // color: white;
							this.logFile +=
								'INFO Started project successfully' + '\n';
							this.debugFile +=
								'INFO Started project successfully' + '\n';
						} catch (e) {
					...
					case 'stop':
						try {
							...
							this.logFile +=
								'INFO Stopped project successfully' + '\n';
							this.debugFile +=
								'INFO Stopped project successfully' + '\n';
						} catch (e) {
							this._addLog(e, `color: ${this.colors.error};`); // color: red;
					...
					case 'pause':
						try {
							...
							this._addLog(
								'Paused project successfully',
								`color: ${this.colors.log};`
							); // color: white;
							this.logFile +=
								'INFO Paused project successfully' + '\n';
							this.debugFile +=
								'INFO Paused project successfully' + '\n';
						} catch (e) {
							this._addLog(e, `color: ${this.colors.error};`); // color: red;
							...
							this.logFile += 'ERROR ' + Cast.toString(e) + '\n';
							this.logFile +=
								'INFO Failed to pause project' + '\n';
							this.debugFile +=
								'ERROR ' + Cast.toString(e) + '\n';
							this.debugFile +=
								'INFO Failed to pause project' + '\n';
						}
						break;
					case 'resume':
						try {
							...
							this.logFile +=
								'INFO Resumed project successfully' + '\n';
							this.debugFile +=
								'INFO Resumed project successfully' + '\n';
						} catch (e) {
							this._addLog(
					...
					case 'broadcast':
						try {
							...
							if (
								this.isProtected({
									VARIABLE: args[0],
									PROTECTION_TYPE: 'Broadcast',
								})
							) {
								this._addLog(
									'This broadcast is protected!',
									`color: ${this.colors.warning};`
								); // color: yellow;
								this.logFile +=
									'WARN This broadcast is protected!' + '\n';
								this.debugFile +=
									'WARN This broadcast is protected!' + '\n';
								break;
							}
							...
							console.log(
								'Broadcasted ' + broadcast + ' successfully'
							);
							this._addLog(
								'Broadcasted ' + broadcast + ' successfully',
								`color: ${this.colors.log};`
					...
					case 'getVar':
						try {
							if (
								this.isProtected({
									VARIABLE: args[1],
									PROTECTION_TYPE: 'Sprite',
								})
							) {
								this._addLog(
									'This sprite is protected!',
									`color: ${this.colors.warning};`
								); // color: yellow;
								this.logFile +=
									'WARN This sprite is protected!' + '\n';
								this.debugFile +=
									'WARN This sprite is protected!' + '\n';
								break;
							}
							let target =
								args[1] === undefined
									...
									: runtime.getSpriteTargetByName(args[1]);
							if (!target) {
								throw new ReferenceError(
									'Target "' + args[1] + '" is not defined'
								);
							}
							...
							if (!variable) {
								throw new ReferenceError(
									'Variable "' + args[0] + '" is not defined'
								);
							}
							if (
								this.isProtected({
									VARIABLE: args[0],
									PROTECTION_TYPE: 'Variable',
								})
							) {
								this._addLog(
								...
								this.logFile +=
									'WARN This variable is protected!' + '\n';
								this.debugFile +=
									'WARN This variable is protected!' + '\n';
								break;
							}
					...
					case 'setVar':
						try {
							...
							let target =
								args[2] === undefined
									? runtime.getTargetForStage()
									: runtime.getSpriteTargetByName(args[2]);
							if (!target) {
								throw new ReferenceError(
									'Target "' + args[2] + '" is not defined'
								);
							}
							const variable = target.lookupVariableByNameAndType(
								Scratch.Cast.toString(args[0]),
					...
					case 'getList':
						try {
							...
							if (!variable) {
								throw new ReferenceError(
									'List "' + args[0] + '" is not defined'
								);
							}
							if (
								this.isProtected({
									VARIABLE: args[0],
									PROTECTION_TYPE: 'List',
								})
							) {
								this._addLog(
					...
					case 'enableDebugMode':
						try {
							...
							this.logFile += 'ERROR ' + Cast.toString(e) + '\n';
							this.logFile +=
								'INFO Failed to enable debug mode' + '\n';
							this.debugFile +=
								'ERROR ' + Cast.toString(e) + '\n';
							this.debugFile +=
								'INFO Failed to enable debug mode' + '\n';
					...
					case 'disableDebugMode':
						try {
							...
							this.logFile += 'ERROR ' + Cast.toString(e) + '\n';
							this.logFile +=
								'INFO Failed to disable debug mode' + '\n';
							this.debugFile +=
								'ERROR ' + Cast.toString(e) + '\n';
							this.debugFile +=
								'INFO Failed to disable debug mode' + '\n';
					...
					case 'downloadLogs':
						const file =
							args[0] === 'true' ? this.debugFile : this.logFile;
						downloadBlob(new Blob([file]), 'project.log');
						break;
					default:
						if (!customCommandExists) {
							if (!(command in this.commandSet)) {
								...
								this.logFile +=
									`INFO Command "${command}" not found. Check "help" for command list.` +
									'\n';
								break;
							}
							try {
			...
			whenCommandEntered({ COMMAND }, util) {
				...
				if (
					!thread.commandData ||
					thread.commandData.COMMAND !== COMMAND
				) {
					thread._reject();
					return false;
				}
			...
			async runWhenCommandEntered(args) {
				const hatsMatch = await new Promise((resolve) => {
					const hats = new Set(
						runHats('debugging_whenCommandEntered')
					);
			...
			cloneMarker({ MARK }, util) {
				...
				if (!this.isClone(clone))
					throw new Error('The main sprite can not be marked!');
				if (clone.marker)
					throw new Error('Clone has already been marked!');
			...
			marker({ MARK }, util) {
				const thread = util.thread;
				if (thread.marker)
					throw new Error('Thread has already been marked!');
			...
			pointer({ POINT }, util) {
				...
				if (!thread.marker)
					throw new Error(
						'Thread needs to be marked before a pointer can be created.'
					);
			...
			sendInput({ COMMAND, LOG }) {
				...
				console.log(parsed);
				(async (THIS, parsed) => await THIS._runCommand(parsed))(
					this,
					parsed
				);
			}
			...
			error({ INFO }, util) {
				...
				const eventBlock = this._findBlockFromId(
					thread.topBlock,
					target
				);
				if (eventBlock) {
			...
			fatalError({ INFO }, util) {
				...
				const text =
					`Fatal Error: ${Cast.toString(INFO)}` +
					`\n${stack.map((text) => `\tat ${text}`).join('\n')}`;
				console.error(text);
			...
			breakPoint() {
				this.openDebugger();
				this._addLog(
					'The project has paused itself, use the resume command to unpause the project.',
					`color: ${this.colors.breakpoint};`
				); // color: cadetblue;
				this.runtime.pause();
			}
			...
			logStage() {
				new Promise((resolve) => {
					...
					this._addImgLog(
						base64ImageData,
						'border-radius: 10px; width: 150px; margin: 10px;'
					);
				});
			}
			logImage({ IMAGE }) {
			...
			test({ TEST, TEST_NAME }) {
				if (TEST) {
					this._addLog(
						...
						`color: ${this.colors.succeededTest};`
					); // color: mediumseagreen;
				} else {
					this._addLog(
						`Test: ${TEST_NAME} failed`,
						`color: ${this.colors.failedTest};`
					); // color: crimson;
				}
			}
			...
			setHelp({ COMMAND, EXPLANATION }) {
				this.CommandDescriptions[Cast.toString(COMMAND)] =
					Cast.toString(EXPLANATION);
			}
			removeHelp({ COMMAND }) {
			...
			setCommandEnabled({ COMMAND, ENABLED }) {
				let commandEnabled = this.isCommandEnabled({
					COMMAND: COMMAND,
				});
				if (
					Cast.toString(ENABLED).toLowerCase() === 'enable' &&
					!commandEnabled
				) {
					this.disabledCommands.splice(
						this.disabledCommands.indexOf(Cast.toString(COMMAND)),
						1
					);
				} else if (
					Cast.toString(ENABLED).toLowerCase() === 'disable' &&
					commandEnabled
				) {
			...
			protect({ PROTECTION_TYPE, VARIABLE }) {
				let variableProtected = this.isProtected({
					VARIABLE: VARIABLE,
				...
				if (!variableProtected) {
					...
					switch (PROTECTION_TYPE) {
						case 'Variable':
							...
							this.protectedVariables.push(
								Cast.toString(VARIABLE)
							);
							break;
						case 'List':
							target = runtime.getTargetForStage();
							...
							break;
						case 'Sprite':
							target = runtime.getSpriteTargetByName(
								Cast.toString(VARIABLE)
							);
							if (!target) {
								throw new ReferenceError(
									'Sprite "' + VARIABLE + '" is not defined'
								);
							}
							this.protectedSprites.push(Cast.toString(VARIABLE));
							break;
						case 'Broadcast':
							this.protectedBroadcasts.push(
								Cast.toString(VARIABLE)
							);
							break;
						default:
			...
			unprotect({ PROTECTION_TYPE, VARIABLE }) {
				...
				if (variableProtected) {
					switch (PROTECTION_TYPE) {
						case 'Variable':
							this.protectedVariables.splice(
								this.protectedVariables.indexOf(
									Cast.toString(VARIABLE)
								),
								1
							);
							break;
						case 'List':
							this.protectedLists.splice(
								this.protectedLists.indexOf(
									Cast.toString(VARIABLE)
								),
								1
							);
							break;
						case 'Sprite':
							this.protectedSprites.splice(
								this.protectedSprites.indexOf(
									Cast.toString(VARIABLE)
								),
								1
							);
							break;
						case 'Broadcast':
							this.protectedBroadcasts.splice(
								this.protectedBroadcasts.indexOf(
									Cast.toString(VARIABLE)
								),
								1
							);
			...
			isProtected({ PROTECTION_TYPE, VARIABLE }) {
				switch (PROTECTION_TYPE) {
					case 'Variable':
						return this.protectedVariables.includes(
							Cast.toString(VARIABLE)
						);
					case 'List':
						return this.protectedLists.includes(
							Cast.toString(VARIABLE)
						);
					case 'Sprite':
						return this.protectedSprites.includes(
							Cast.toString(VARIABLE)
						);
					case 'Broadcast':
						return this.protectedBroadcasts.includes(
							Cast.toString(VARIABLE)
						);
					default:
						throw new Error('Invalid PROTECTION_TYPE');
			...
			setColorOf({ STYLE_TYPE, COLOR }) {
				switch (Cast.toString(STYLE_TYPE).toLowerCase()) {
					...
					case 'input text':
						...
						this.consoleBarInput.style.color =
							this.colors.inputText;
						this.consoleBarIndicator.style.color =
							this.colors.inputText;
						break;
			...
			canScroll() {
				return (
					Math.abs(
						this.consoleLogs.scrollHeight -
							this.consoleLogs.clientHeight -
							this.consoleLogs.scrollTop
					) <= 1
				);
			}
		}
